...
class BeerStore {
    ...
    var allBeers: Variable<[Beer]> = Variable([])
...
//            }
    ...
    func updateBreweryDetails(onCompletion: @escaping () -> Void) {
        ...
        BrewerydbAPI.sharedInstance.get_beers_breweries(beers: allBeers, onCompletion: { (updatedBeers: Variable<[Beer]>) in
            self.allBeers = updatedBeers
            onCompletion()
        })
    }
